...
@Composable
fun ExecutionView(architecture: Architecture?) {

    val disassembler = architecture?.disassembler

    val codeStyle = FontType.CODE.getStyle()
    val baseStyle  = FontType.MEDIUM.getStyle()
    ...
    if (disassembler == null) {
        Box(Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            CLabel(text = "Disassembler missing!")
        }
    } else {
        ...
        Row(Modifier.fillMaxSize()) {
            ...
            Column(Modifier.fillMaxSize()) {
                ...
                Column(Modifier.fillMaxSize().verticalScroll(vScrollState)) {
                    disassembler.decoded.value.filter { it.decodedContent.isNotEmpty() }.forEach { segment ->

                        segment.decodedContent.forEach { decoded ->
                            ...
                            // Store the row position based on the Y offset
                        }

                        Box(Modifier.fillMaxWidth(), contentAlignment = Alignment.Center) {
                            Text("...", fontFamily = codeStyle.fontFamily, fontSize = codeStyle.fontSize, color = theme.COLOR_FG_0)
                        }
                    }
                }
            }
